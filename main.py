...
def draw_temp (temp, dewp):
    ...
    while len(temp) > 0:
        t1 = temp.pop()
        h1 = temp.pop()
        shape_temp = [(border + t_dist * t0 *0.1 + h0/hmax * tx + t_dist * offset, border + ty - h0 / hmax * ty), (border + t_dist * t1 *0.1 + h1/hmax * tx + t_dist * offset, border + ty - h1 / hmax * ty)]
        img1.line(shape_temp, fill ="red", width = 3)
        d1 = dewp.pop()
        dh1 = dewp.pop()
        shape_temp = [(max((border + t_dist * d0 *0.1 + dh0/hmax * tx + t_dist * offset), border), border + ty - dh0 / hmax * ty), (max((border + t_dist * d1 *0.1 + dh1/hmax * tx + t_dist * offset), border), border + ty - dh1 / hmax * ty)]
...
def draw_wind(wind):
    while len(wind) > 0:
        ...
        img1.ellipse((tx + border - wind_dot, ty - hight / hmax * ty - wind_dot + border, tx + border + wind_dot, ty - hight / hmax * ty + wind_dot + border), fill=dot_color)
        img1.text((tx + border - wind_dot/2, ty - hight / hmax * ty - wind_dot/2 + border), str(strength) ,(20,20,20), font=font)
