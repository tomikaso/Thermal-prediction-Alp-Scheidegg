...
class thermal_low:
    ...
    # estimate the snow frontier by the date
        ...
        while day < 5:
            time = 0
            while time <= 10:
                self.html_string.append('DAY' + str(day) + 'LT' + str(time + 10) + ',')
                value = sum(self.thermal_activity[max(0, (time - 3)) + day * 11: time + day * 11]) / 600
                self.html_string.append(str(round(value, 2)) + ',')
                time += 1
            day += 1
        print('HTML-String: ')
